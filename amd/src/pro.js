...
define(['jquery', 'core/log', 'core/config'], function($, log, cfg) {
    ...
    var blkdashboardpro = {
        ...
        init: function() {
            ...
            $('#id_s_block_dashboard_licensekey').bind('change', this.check_product_key);
            $('#id_s_block_dashboard_licensekey').trigger('change');
        ...
        check_product_key: function() {
            ...
            var calculated = blkdashboardpro.checksum(payload);
            ...
            if (crc === calculated) {
                var url = cfg.wwwroot + '/blocks/dashboard/pro/ajax/services.php?';
                url += 'what=license';
                url += '&service=check';
                url += '&customerkey=' + that.val();
                url += '&provider=' + $('#id_s_block_dashboard_licenseprovider').val();

                $('#id_s_block_dashboard_licensekey + img').remove();
                $('#id_s_block_dashboard_licensekey').after(waiticon);

                $.get(url, function(data) {
                    if (data.match(/SET OK/)) {
                        $('#id_s_block_dashboard_licensekey + img').remove();
                        $('#id_s_block_dashboard_licensekey').after(validicon);
                    } else {
                        $('#id_s_block_dashboard_licensekey + img').remove();
                        $('#id_s_block_dashboard_licensekey').after(invalidicon);
                    }
                }, 'html');
            } else {
                $('#id_s_block_dashboard_licensekey + img').remove();
                $('#id_s_block_dashboard_licensekey').after(cautionicon);
            }
        },
