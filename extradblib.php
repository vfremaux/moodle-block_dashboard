...
function extra_db_set_config($configkey, $value){
	...
	set_config($configkey, null, $value);
}
...
function extra_db_connect($return = false, &$error){
	...
	$extra_dbport = extra_db_get_config('block_dashboard_extra_db_port');
	if (empty($extra_dbport)){
		extra_db_set_config('block_dashboard_extra_db_port', 5432); // Default port for PostGre
	}

	$extra_dbhost = extra_db_get_config('block_dashboard_extra_dbhost');
	if (empty($extra_dbhost)){
		extra_db_set_config('block_dashboard_extra_db_host', 'localhost'); // Default host for PostGre
	}

	$extra_dbdb = extra_db_get_config('block_dashboard_extra_db_db');
	if (empty($extra_dbdb)){
		extra_db_error("extra_db needs a DB"); // Default host for PostGre
	}

	$cnxstring = ' host='.extra_db_get_config('block_dashboard_extra_db_host').' port='.extra_db_get_config('block_dashboard_extra_db_port').' dbname='.extra_db_get_config('block_dashboard_extra_db_db').' user='.extra_db_get_config('block_dashboard_extra_db_user').' password='.extra_db_get_config('block_dashboard_extra_db_password');
